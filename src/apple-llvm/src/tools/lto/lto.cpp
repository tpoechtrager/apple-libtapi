...
void lto_codegen_reset_context(lto_code_gen_t cg) {
  ...
  assert(CG->OwnedContext.get() != NULL && "Context not owned by CG");
  std::unique_ptr<LLVMContext> context = make_unique<LLVMContext>();
  CG->resetContext(context.get());
  std::swap(CG->OwnedContext, context);
}
