...
DwarfAccelTable::HashDataContents::~HashDataContents() {}

template <bool IsType>
class DIEHashDataContents : public DwarfAccelTable::HashDataContents {
  const DIE *Die;
...
public:
  ...
  bool less(const HashDataContents *HD) const override {
    const auto *Other = static_cast<const DIEHashDataContents *>(HD);
...
static LLVM_CONSTEXPR DwarfAccelTable::Atom DieAtoms[] = {
...
struct ModuleHashDataContents : public DwarfAccelTable::HashDataContents {
  DwarfStringPoolEntryRef Module;

  ModuleHashDataContents(DwarfStringPoolEntryRef Module) : Module(Module) {}

  void emit(AsmPrinter *Asm, DwarfDebug */* D */) const override {
    // Emit the string offset
    Asm->EmitInt32(Module.getOffset());
  }

  bool less(const HashDataContents *HD) const override {
    const auto *Other = static_cast<const ModuleHashDataContents *>(HD);
    return Module.getOffset() < Other->Module.getOffset();
  }

  void print(raw_ostream &OS) const override {
    OS << "  Module string offset: " << Module.getOffset() << "\n";
  }
};

ModuleDwarfAccelTable::ModuleDwarfAccelTable()
    : DwarfAccelTable(DwarfAccelTable::Atom(dwarf::DW_ATOM_ext_types,
                                            dwarf::DW_FORM_data4)) {}

void ModuleDwarfAccelTable::AddUID(StringRef UID,
                                   DwarfStringPoolEntryRef UIDEntry,
                                   DwarfStringPoolEntryRef ModuleEntry) {
  assert(Data.empty() && "Already finalized!");
  // If the string is in the list already then add this die to the list
  // otherwise add a new one.
  DataArray &Entry = Entries[UID];
  assert(!Entry.Name || Entry.Name == UIDEntry);
  Entry.Name = UIDEntry;
  if (Entry.Values.size())
    assert(static_cast<ModuleHashDataContents *>(Entry.Values[0])->Module == ModuleEntry && "UID entry is not unique");
  else
    Entry.Values.push_back(new (Allocator) ModuleHashDataContents(ModuleEntry));
}
